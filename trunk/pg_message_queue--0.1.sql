...
BEGIN
   SELECT channel INTO t_channel FROM pg_mq_queue_catalog
...
RETURNS pg_mq_queue_catalog
LANGUAGE PLPGSQL VOLATILE SECURITY DEFINER AS $$

DECLARE 
    out_val pg_mq_queue_catalog%ROWTYPE;
    t_table_name name;
BEGIN

t_table_name := 'pg_mq_queue_' || in_channel;

INSERT INTO pg_mq_queue_catalog (table_name, channel, payload_type)
VALUES (t_table_name, in_channel, in_type);

SELECT * INTO out_val FROM pg_mq_queue_catalog 
 WHERE channel = in_channel;

EXECUTE 'CREATE TABLE ' || quote_ident(t_table_name) || '(
    PRIMARY KEY (msg_id)
  ) INHERITS (' || quote_ident('pg_mq_' || in_type ) || ')';
...
BEGIN
   ...
   SELECT table_name INTO t_table_name FROM pg_mq_queue_catalog
    WHERE channel = in_channel;

   EXECUTE 'DROP TABLE ' || quote_ident(t_table_name) || ' CASCADE';

   DELETE FROM pg_mq_queue_catalog WHERE channel = in_channel;
...
LANGUAGE PLPGSQL VOLATILE AS $$
    DECLARE cat_entry pg_mq_queue_catalog%ROWTYPE;
            out_val pg_mq_base%ROWTYPE;
    BEGIN
       SELECT * INTO cat_entry FROM pg_mq_queue_catalog
        WHERE channel = in_channel;
...
LANGUAGE PLPGSQL VOLATILE AS $$
   ...
   BEGIN
      SELECT * INTO cat_entry FROM pg_mq_queue_catalog
        WHERE channel = in_channel;
      IF cat_entry.payload_type <> 'text' THEN
...
CREATE OR REPLACE FUNCTION pg_mq_get_msg_xml(in_channel name, in_num_msgs int)
RETURNS SETOF pg_mq_xml
LANGUAGE PLPGSQL AS $$
   DECLARE cat_entry pg_mq_queue_catalog%ROWTYPE;
   BEGIN
      SELECT * INTO cat_entry FROM pg_mq_queue_catalog
        WHERE channel = in_channel;
      IF cat_entry.payload_type <> 'xml' THEN
         RAISE EXCEPTION 'Incorrect Type Called';
      END IF;
      RETURN QUERY EXECUTE
         $e$ UPDATE $e$ || quote_ident(cat_entry.table_name) || $e$
